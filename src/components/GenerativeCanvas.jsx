...
const SPAWN_RATE = 5
const BOT_COLORS = ['#FF0055', '#00FF88', '#0055FF', '#FFBB00', '#2C2C2C']
...
class Particle {
  ...
  update(globalTurbulence) {
    const angle = this.x * 0.01 + this.y * 0.01 + performance.now() * 0.001
...
function makeBotParticle(x, y, vx, vy, color) {
  return new Particle(x, y, vx, vy, color, 1 + Math.random() * 3, 10)
}
...
export function GenerativeCanvas() {
  ...
  useEffect(() => {
    ...
    s.bots = Array.from({ length: 3 }, () => ({
      x: Math.random() * s.width,
      y: Math.random() * s.height,
      vx: 0,
      vy: 0,
      targetX: Math.random() * s.width,
      targetY: Math.random() * s.height,
    ...
    function updateBots() {
      s.bots.forEach((bot) => {
        ...
        bot.vx += dx * 0.001
        bot.vy += dy * 0.001
        const speed = Math.sqrt(bot.vx ** 2 + bot.vy ** 2)
        if (speed > 5) {
        ...
        bot.y += bot.vy
        if (Math.random() < 0.01) {
          bot.targetX = Math.random() * s.width
          bot.targetY = Math.random() * s.height
          bot.color = BOT_COLORS[Math.floor(Math.random() * BOT_COLORS.length)]
        }
        if (s.particles.length < MAX_PARTICLES && Math.random() < 0.5) {
          s.particles.push(makeBotParticle(bot.x, bot.y, bot.vx * 0.5, bot.vy * 0.5, bot.color))
        }
      })
    }
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.status}>
        ...
        <span className={styles.statusSub}>DECAY RATE: 0.05 / FRAME</span>
      </div>
    </div>
  )
}
...
function ParticleCount({ stateRef }) {
  ...
  useEffect(() => {
    let rafId
    function tick() {
      setCount(stateRef.current.particles.length)
